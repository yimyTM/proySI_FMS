...
const createUser = async (req, res) => {
  ...
  if (!username || !password || !id_rol || !email) {
    return res
      ...
      .json({
        message:
          "Todos los campos obligatorios (username, email, password e id_rol) deben estar llenos.",
      });
  }
  ...
  try {
    ...
    if (rolExistente.rows.length === 0) {
      return res
        .status(400)
        .json({ message: "El rol especificado no existe en el sistema." });
    }
...
const updateUser = async (req, res) => {
  ...
  if (!username || !id_rol || estado === undefined || !email) {
    return res
      ...
      .json({
        message:
          "Debe enviar username, email, id_rol y estado para actualizar el usuario.",
      });
  }

  try {
    const usuarioExistente = await pool.query(
      "SELECT id_usuario FROM usuario WHERE (username = $1 OR email = $2) AND id_usuario != $3",
      [username, email, id],
    );
    if (usuarioExistente.rows.length > 0) {
      return res
        ...
        .json({
          message:
            "El nombre de usuario o correo electrónico ya está en uso por otra cuenta.",
        });
    }
...
const deleteUser = async (req, res) => {
  ...
  try {
    ...
    if (desactivedUser.rows.length == 0)
      return res.status(404).json({ message: "Usuario no encontrado." });

    res.json({
      message: "Usuario desactivado correctamente.",
      user: desactivedUser.rows[0],
    });
  } catch (error) {
    res
      .status(500)
